\documentclass[14pt]{extarticle}
\usepackage[utf8]{inputenc}
\usepackage[english, russian]{babel}
\usepackage{latexsym,amsfonts,amssymb,amsthm,amsmath,graphicx,array,tabularx,tabulary,booktabs,longtable,multirow,wrapfig, longtable,float,hyperref}
\usepackage[dvipsnames]{xcolor}
\linespread{1.3}

\hypersetup{				% Гиперссылки
	unicode=true,           % русские буквы в раздела PDF
	pdftitle={Заголовок},   % Заголовок
	pdfauthor={Автор},      % Автор
	pdfsubject={Тема},      % Тема
	pdfcreator={Создатель}, % Создатель
	pdfproducer={Производитель}, % Производитель
	pdfkeywords={keyword1} {key2} {key3}, % Ключевые слова
	colorlinks=true,       	% false: ссылки в рамках; true: цветные ссылки
	linkcolor=blue,          % внутренние ссылки
	citecolor=blue,        % на библиографию
	filecolor=magenta,      % на файлы
	urlcolor=red           % на URL
}



\renewcommand{\labelenumi}{\arabic{enumi}.}
\renewcommand{\labelenumii}{\arabic{enumi}.\arabic{enumii}}


\DeclareMathOperator{\sgn}{\mathop{sgn}}

\newcommand*{\hm}[1]{#1\nobreak\discretionary{}
	{\hbox{$\mathsurround=0pt #1$}}{}}


\setlength\fboxsep{3pt}
\setlength\fboxrule{1pt} 



\usepackage{geometry} 
\geometry{top=25mm}
\geometry{bottom=35mm}
\geometry{left=15mm}
\geometry{right=15mm}

\newread\tmp

\newcommand{\quickcharcount}[1]{%
	\immediate\write18{texcount -1 -sum -merge -char #1.tex > #1-chars.sum}%
	\openin\tmp=#1-chars.sum%
	\read\tmp to \thechar%
	\closein\tmp%
}

\newcommand{\quickwordcount}[1]{%
	\immediate\write18{texcount -1 -sum -merge #1.tex > #1-words.sum}%
	\openin\tmp=#1-words.sum%
	\read\tmp to \theword%
	\closein\tmp%
}
\usepackage{subfiles}
\usepackage{indentfirst}
\setlength{\oddsidemargin}{0in}
\setlength{\textwidth}{6.5in}
\setlength{\textheight}{8.4in}
\setlength{\topmargin}{0in}
\setlength{\headheight}{10pt}
\setlength\parindent{24pt}

\graphicspath{{Pictures/}}
\DeclareGraphicsExtensions{.pdf,.png,.jpg}

\usepackage{caption}
\captionsetup{justification=raggedright,singlelinecheck=false}

\usepackage[backend=biber, style=apa]{biblatex}


\addbibresource{bible.bib}






\begin{document}
	
	
	\thispagestyle{empty} 
	
	\begin{center}
		NATIONAL RESEARCH UNIVERSITY
		\\
		HIGHER SCHOOL OF ECONOMICS
		\\
		FACULTY OF SOCIAL SCIENCE
		
	\end{center}
	\vspace{12ex}
	
	\begin{center}
		\textbf{MEGA PAPER}\\
		\vspace{2ex}
		\textbf{PZDataAnalysis}\\
		\vspace{2ex}
		Political and Physical Science
	\end{center}
	
	\vspace{5ex}
	\begin{flushright}
		\hspace{40ex}
		Student\\
		Anastasia Uspenskaya \\
		Arina Puchkova \\
		Sergey Zakharov\\
		Igor Kolesnikov\\
		
		\vspace{5ex}
		Spasibo chto est'\\
		Evgeniy Sokolov \\
		
		
		
	\end{flushright}
	
	\begin{center}
		\vfill
		Moscow, 2021
	\end{center}
	
	\newpage
	
	
	\tableofcontents
	
	\newpage
	
	
	\section{Опишите две компоненты, из которых состоит perceptual loss (стилевую и контентную). Как можно использовать этот функционал, чтобы делать перенос стиля?}
	\input{TeX_files/1.tex}
	
	\newpage
	\section{Как можно сделать перенос стиля более быстрым, если стилевое изображение известно и не будет меняться?} % S
	\input{TeX_files/2.tex}
	
	\newpage
	\section{Что в вариационном автокодировщике выдают кодировщик и декодировщик?} 
	\input{TeX_files/3.tex}
	
	\newpage
	\section{Как устроен функционал ошибки в вариационном автокодировщике? За что отвечает каждое из слагаемых?} % Arina
	\input{TeX_files/4.tex}
	
	\newpage
	\section{В чём состоит идея трюка репараметризации (reparametrization trick)?} % S
	\input{TeX_files/5.tex}
	
	\newpage
	\section{Запишите функцию потерь GAN. Как в ней обозначается дискриминатор и генератор? Как она используется при обучении (ищем минимум, максимум или как-то еще)?} % Arina
	\input{TeX_files/6.tex}
	
	\newpage
	\section{Что такое затухание градиентов в GAN? Покажите это математически.} % S
	\input{TeX_files/7.tex}
	
	\newpage
	
	\section{Запишите теорему о замене переменных для функции плотности распределения случайной величины. Что значит каждое обозначение в ней?} % S
	\input{TeX_files/8.tex}
	
	\newpage
	
	\section{Запишите функцию преобразования для Real-NVP нормализационного потока. Как посчитать ее Якобиан? Как выглядит обратная функция?} % S
	\input{TeX_files/9.tex}
	
	\newpage
	
	\section{Что такое мел-спектрограмма? Как она получается из wav-файла?} % Arina
	\input{TeX_files/10.tex}
	
	\newpage
	
	\section{Какие метрики качества ASR вы знаете? Как они вычисляются?}
	\input{TeX_files/11.tex}
	
	\newpage
	
	\section{Что такое MuLaw-кодирование, для чего оно нужно и как оно вычисляется?}
	\input{TeX_files/12.tex}
	
	\newpage
	
	\section{Как устроен WaveNet? Почему он медленно работает на этапе применения?}
	\input{TeX_files/13.tex}
	
	\newpage
	
	\section{Что является обучающей выборкой в задаче рекомендаций? Что такое рейтинг? Приведите примеры, как он может задаваться.}
	\input{TeX_files/14.tex}
	
	\section{Какие вы знаете метрики качества рекомендаций? Как можно измерить ошибку, если рейтинги вещественные? Если рейтинги — это классы? Как учесть, что нам важен порядок, в котором задаются объекты? Как устроена метрика nDCG?} % Arina
	\input{TeX_files/15.tex}
	
	\newpage
	
	\section{Как устроены user-based рекомендации?}  
	\input{TeX_files/16.tex}
	
	\newpage
	
	\section{Что такое модель со скрытыми переменными (LFM)? Как в ней предсказывается рейтинг? Как устроен функционал, на который она обучается?} % Arina
	\input{TeX_files/17.tex}
	
	\newpage
	
	\section{Как устроена модель iALS?} 
	\input{TeX_files/18.tex}
	
	\newpage
	
	\section{Как обычно устроена рекомендательная система? Опишите шаги отбора кандидатов, ранжирования, переранжирования. Приведите примеры, как каждый из них может быть устроен} 
	\input{TeX_files/19.tex}
\end{document}
